...
export default function Copilot({ analysis }) {
  ...
  return <aside className={`copilot ${open ? 'expanded' : ''}`} aria-live="polite">
    {open && <div className="copilot-panel"><div className="copilot-head"><span><Bot size={19}/><b>ThelivLens Copilot</b><small>Ask questions about this analysis.</small></span><button onClick={() => setOpen(false)} aria-label="Close Copilot"><X size={18}/></button></div><div className="chat-body">{messages.length === 0 && <><p className="ai-message">I’m here to make the evidence easier to understand.</p><div className="suggestions">{Object.keys(replies).map(q => <button key={q} onClick={() => ask(q)}>{q}</button>)}</div></>}{messages.map((m,i) => <p key={i} className={`${m.who}-message`}>{m.text}</p>)}</div><form className="chat-input" onSubmit={e => {e.preventDefault(); ask(input)}}><input value={input} onChange={e=>setInput(e.target.value)} placeholder="Ask about this analysis" aria-label="Ask the Copilot"/><button aria-label="Send message"><ArrowUp size={17}/></button></form></div>}
    <button className="copilot-trigger" onClick={() => setOpen(!open)}>{open ? <X size={20}/> : <><Sparkles size={18}/><span>Ask Copilot</span></>}</button>
  </aside>;
}
